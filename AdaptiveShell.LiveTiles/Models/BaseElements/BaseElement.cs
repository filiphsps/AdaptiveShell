...
namespace AdaptiveShell.LiveTiles.Models.BaseElements {
    public abstract class BaseElement : BindableBase {
        /// <summary>
        ...
        internal Boolean TryParseEnum<TEnum>(String text, out TEnum answer) where TEnum : struct, IConvertible {
            Array supportedEnums = this.GetSupportedEnums<TEnum>();

            if (text != null) {
                foreach (Object e in supportedEnums) {
        ...
        public void ApplyDataBindingToThisInstance(DataBindingValues dataBinding) {
            foreach (ConfiguredBinding binding in this._bindings) {
                String value;
